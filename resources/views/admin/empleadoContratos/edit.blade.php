...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.empleadoContrato.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.empleado-contratos.update", [$empleadoContrato->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('empleado') ? 'is-invalid' : '' }}" name="empleado_id" id="empleado_id" required>
                    @foreach($empleados as $id => $entry)
                        <option value="{{ $id }}" {{ (old('empleado_id') ? old('empleado_id') : $empleadoContrato->empleado->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('contrato_laboral') ? 'is-invalid' : '' }}" name="contrato_laboral_id" id="contrato_laboral_id" required>
                    @foreach($contrato_laborals as $id => $entry)
                        <option value="{{ $id }}" {{ (old('contrato_laboral_id') ? old('contrato_laboral_id') : $empleadoContrato->contrato_laboral->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_asignacion') ? 'is-invalid' : '' }}" type="text" name="fecha_asignacion" id="fecha_asignacion" value="{{ old('fecha_asignacion', $empleadoContrato->fecha_asignacion) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_retiro') ? 'is-invalid' : '' }}" type="text" name="fecha_retiro" id="fecha_retiro" value="{{ old('fecha_retiro', $empleadoContrato->fecha_retiro) }}">
