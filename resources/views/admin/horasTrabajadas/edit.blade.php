...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.horasTrabajada.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.horas-trabajadas.update", [$horasTrabajada->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('empleado') ? 'is-invalid' : '' }}" name="empleado_id" id="empleado_id" required>
                    @foreach($empleados as $id => $entry)
                        <option value="{{ $id }}" {{ (old('empleado_id') ? old('empleado_id') : $horasTrabajada->empleado->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('contrato_laboral') ? 'is-invalid' : '' }}" name="contrato_laboral_id" id="contrato_laboral_id" required>
                    @foreach($contrato_laborals as $id => $entry)
                        <option value="{{ $id }}" {{ (old('contrato_laboral_id') ? old('contrato_laboral_id') : $horasTrabajada->contrato_laboral->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha') ? 'is-invalid' : '' }}" type="text" name="fecha" id="fecha" value="{{ old('fecha', $horasTrabajada->fecha) }}" required>
                @if($errors->has('fecha'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('horas') ? 'is-invalid' : '' }}" type="number" name="horas" id="horas" value="{{ old('horas', $horasTrabajada->horas) }}" step="0.01" required>
                @if($errors->has('horas'))
