...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.asistenciaEntrenamiento.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.asistencia-entrenamientos.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('entrenamientos') ? 'is-invalid' : '' }}" name="entrenamientos[]" id="entrenamientos" multiple>
                    @foreach($entrenamientos as $id => $entrenamiento)
                        <option value="{{ $id }}" {{ in_array($id, old('entrenamientos', [])) ? 'selected' : '' }}>{{ $entrenamiento }}</option>
                    @endforeach
                </select>
