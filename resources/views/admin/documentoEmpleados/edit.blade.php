...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.documentoEmpleado.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.documento-empleados.update", [$documentoEmpleado->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('empleado') ? 'is-invalid' : '' }}" name="empleado_id" id="empleado_id" required>
                    @foreach($empleados as $id => $entry)
                        <option value="{{ $id }}" {{ (old('empleado_id') ? old('empleado_id') : $documentoEmpleado->empleado->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tipo_documento') ? 'is-invalid' : '' }}" name="tipo_documento" id="tipo_documento" required>
                    ...
                    @foreach(App\Models\DocumentoEmpleado::TIPO_DOCUMENTO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tipo_documento', $documentoEmpleado->tipo_documento) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_emision') ? 'is-invalid' : '' }}" type="text" name="fecha_emision" id="fecha_emision" value="{{ old('fecha_emision', $documentoEmpleado->fecha_emision) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_vencimiento') ? 'is-invalid' : '' }}" type="text" name="fecha_vencimiento" id="fecha_vencimiento" value="{{ old('fecha_vencimiento', $documentoEmpleado->fecha_vencimiento) }}">
