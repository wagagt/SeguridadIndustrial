...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.asistenciaCharla.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.asistencia-charlas.update", [$asistenciaCharla->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <div class="form-check {{ $errors->has('presente') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="presente" value="0">
                    <input class="form-check-input" type="checkbox" name="presente" id="presente" value="1" {{ $asistenciaCharla->presente || old('presente', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('charlas') ? 'is-invalid' : '' }}" name="charlas[]" id="charlas" multiple>
                    @foreach($charlas as $id => $charla)
                        <option value="{{ $id }}" {{ (in_array($id, old('charlas', [])) || $asistenciaCharla->charlas->contains($id)) ? 'selected' : '' }}>{{ $charla }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('empleados') ? 'is-invalid' : '' }}" name="empleados[]" id="empleados" multiple>
                    @foreach($empleados as $id => $empleado)
                        <option value="{{ $id }}" {{ (in_array($id, old('empleados', [])) || $asistenciaCharla->empleados->contains($id)) ? 'selected' : '' }}>{{ $empleado }}</option>
                    @endforeach
                </select>
