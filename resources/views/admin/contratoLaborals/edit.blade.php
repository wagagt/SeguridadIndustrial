...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.contratoLaboral.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.contrato-laborals.update", [$contratoLaboral->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="numero_contrato">{{ trans('cruds.contratoLaboral.fields.numero_contrato') }}</label>
                <input class="form-control {{ $errors->has('numero_contrato') ? 'is-invalid' : '' }}" type="text" name="numero_contrato" id="numero_contrato" value="{{ old('numero_contrato', $contratoLaboral->numero_contrato) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('descripcion') ? 'is-invalid' : '' }}" name="descripcion" id="descripcion">{{ old('descripcion', $contratoLaboral->descripcion) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_inicio') ? 'is-invalid' : '' }}" type="text" name="fecha_inicio" id="fecha_inicio" value="{{ old('fecha_inicio', $contratoLaboral->fecha_inicio) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_fin') ? 'is-invalid' : '' }}" type="text" name="fecha_fin" id="fecha_fin" value="{{ old('fecha_fin', $contratoLaboral->fecha_fin) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('estado') ? 'is-invalid' : '' }}" name="estado" id="estado" required>
                    ...
                    @foreach(App\Models\ContratoLaboral::ESTADO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('estado', $contratoLaboral->estado) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cliente') ? 'is-invalid' : '' }}" name="cliente_id" id="cliente_id" required>
                    @foreach($clientes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('cliente_id') ? old('cliente_id') : $contratoLaboral->cliente->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
