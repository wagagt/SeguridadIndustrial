...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.contratoDocumento.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.contrato-documentos.update", [$contratoDocumento->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tipo_documento') ? 'is-invalid' : '' }}" name="tipo_documento" id="tipo_documento" required>
                    ...
                    @foreach(App\Models\ContratoDocumento::TIPO_DOCUMENTO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tipo_documento', $contratoDocumento->tipo_documento) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_subida') ? 'is-invalid' : '' }}" type="text" name="fecha_subida" id="fecha_subida" value="{{ old('fecha_subida', $contratoDocumento->fecha_subida) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('contrato') ? 'is-invalid' : '' }}" name="contrato_id" id="contrato_id">
                    @foreach($contratos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('contrato_id') ? old('contrato_id') : $contratoDocumento->contrato->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
