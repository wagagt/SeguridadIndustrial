...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.charla.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.charlas.update", [$charla->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="tema">{{ trans('cruds.charla.fields.tema') }}</label>
                <input class="form-control {{ $errors->has('tema') ? 'is-invalid' : '' }}" type="text" name="tema" id="tema" value="{{ old('tema', $charla->tema) }}" required>
                @if($errors->has('tema'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('instructor') ? 'is-invalid' : '' }}" name="instructor_id" id="instructor_id" required>
                    @foreach($instructors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('instructor_id') ? old('instructor_id') : $charla->instructor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha') ? 'is-invalid' : '' }}" type="text" name="fecha" id="fecha" value="{{ old('fecha', $charla->fecha) }}" required>
                @if($errors->has('fecha'))
