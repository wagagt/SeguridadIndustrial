...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.mantenimiento-herramienta.update", [$mantenimientoHerramientum->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('herramienta') ? 'is-invalid' : '' }}" name="herramienta_id" id="herramienta_id" required>
                    @foreach($herramientas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('herramienta_id') ? old('herramienta_id') : $mantenimientoHerramientum->herramienta->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tipo_mantenimiento') ? 'is-invalid' : '' }}" name="tipo_mantenimiento" id="tipo_mantenimiento" required>
                    ...
                    @foreach(App\Models\MantenimientoHerramientum::TIPO_MANTENIMIENTO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tipo_mantenimiento', $mantenimientoHerramientum->tipo_mantenimiento) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_mantenimiento') ? 'is-invalid' : '' }}" type="text" name="fecha_mantenimiento" id="fecha_mantenimiento" value="{{ old('fecha_mantenimiento', $mantenimientoHerramientum->fecha_mantenimiento) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('descripcion') ? 'is-invalid' : '' }}" type="text" name="descripcion" id="descripcion" value="{{ old('descripcion', $mantenimientoHerramientum->descripcion) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('resultado') ? 'is-invalid' : '' }}" type="text" name="resultado" id="resultado" value="{{ old('resultado', $mantenimientoHerramientum->resultado) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('notificado') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="notificado" id="notificado" value="1" {{ $mantenimientoHerramientum->notificado || old('notificado', 0) === 1 ? 'checked' : '' }}>
